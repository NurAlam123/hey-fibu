...
import type { EmojiTypes } from "../discordTypes";

const Emoji = async ({ name = '', id = '', animated = false, guild_id = '' }: EmojiTypes) => {
    if ((!id && !name) || (name.startsWith(':') && name.endsWith(':') && !guild_id && !sysEmoji.has(name))) {
        return '';
    }

    if (name.startsWith(':') && name.endsWith(':') && guild_id && !sysEmoji.has(name)) {
        name = name.replace(/\:/g, '');
